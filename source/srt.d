...
struct Subtitle {
  struct Builder {
    ...
    Duration convertToDuration(string h, string m, string s, string ms) {
      return dur!("msecs")(ms.to!size_t)
        + dur!("seconds")(s.to!size_t)
        + dur!("minutes")(m.to!size_t)
        + dur!("hours")(h.to!size_t);
    }
    Subtitle* add(string s) {
      if ((s == null) &&
        ...
        Duration startOffset = convertToDuration(m.captures[1],m.captures[2],m.captures[3],m.captures[4]);
        Duration endOffset = convertToDuration(m.captures[5],m.captures[6],m.captures[7],m.captures[8]);
